...
/* If you reading this, remember to add automatically all channels for whitelist instead all channels for common list */
/* We have 3 types of channel:
 * Whitelist: is a channel that everyone can mention the Fhany.
 * Comum: The mention of Fhany is only possible if the Fhany is active in chat, and the chat warn when the Fhany is Active
 * Blacklist: The mention of Fhany is impossible anyway
*/

const fhanyPresenceDetector: IFeature = { 
    active: false,
    ...
    execute(client, config) {
        const thisConfig = config.features.fhanyPresenceDetector
        ...
        client.on('messageCreate', async message => {
            ...
            if(thisConfig.whiteListChannels.includes(message.channel.id)) return;
            if(message.author.id === thisConfig.fhanyId) {
                if(thisConfig.blackListChannels.includes(message.channel.id)) return;
                ...
                if(thisConfig.blackListChannels.includes(message.channel.id)) return deleteUserMessage(message, true);
            ...
            function mentionTheFhany(message: Message) {
                return new Promise<boolean>((terminated) => {
                    const fhany = client.guilds.cache.get(config.guildId)?.members.cache.get(thisConfig.fhanyId);
                    if(!fhany) return;
                    
                    fhany.roles.cache.forEach(role => {
                        message.mentions.users.has(thisConfig.fhanyId) && terminated(true);
            ...
            function deleteUserMessage(message: Message, isBlackList?: boolean) {
                searchUserInCache(message.author).length === 0 
                    && message.reply({ embeds: [isBlackList 
                ...
                searchUserInCache(message.author).length === 1
                    && message.reply({ embeds: [embed.mentionFhanyNotPermitted2(message)] });
            
                searchUserInCache(message.author).length === 2
                    && addSilenceRole(searchUserInCache(message.author)[0])
                    && message.reply({ embeds: [embed.mentionFhanyNotPermitted3(message)] })
        ...
        client.on('typingStart', async typing => {
            if(thisConfig.blackListChannels.includes(typing.channel.id)) return;
            if(thisConfig.whiteListChannels.includes(typing.channel.id)) return;
            if(typing.user.id !== thisConfig.fhanyId) return;
        
            const fetchChannel = client.channels.cache.get(typing.channel.id);
            const count = getLastNumber();
            fhanyActiveCache.length === 0 && warnAllUsersOfFhanyOn(fetchChannel);
...
module.exports = fhanyPresenceDetector;